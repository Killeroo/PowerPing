namespace PowerPing
{
    public class ConsoleMessageHandler : PingEventHandlerBase, IScanEventHandler
    {
        ...
        public void OnReply(PingReply response)
        {
            ...
            if (Attributes != null &&
                ((Attributes.BeepMode == 2 /* Beep on success */ &&
                  response.Packet.Type != 3 /* Host Unreachable packet */) ||
                 (Attributes.BeepMode == 1 /* Beep on error */ && response.Packet.Type == 3)))
            {
                ...
                {
                } // Silently continue if Console.Beep errors
            }
        }
